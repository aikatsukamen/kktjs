...
このプロジェクトは難読化された巨大な実行コードを機械的に TypeScript 化し、Vue 3 + Vite で
ビルドしている。**型チェックとビルドだけでは実行時の不具合（特に機械変換したモジュールの
実行経路や、DOM テンプレートの実行時コンパイル）を捕捉できない**ため、実際の起動・操作は
...
このコンテナでは、バックグラウンドに回した常駐サーバ（`npm run dev &` や `vite preview &` など）が
