...
`index.html`（プロジェクトルート）に inline で書かれている Vue テンプレート（x-template モーダル群と
...
`index.html` は **約3,790行**。テンプレートの主な構成要素（実測）:
...
### 7. ビルド構成（Vite 標準エントリ・Vue は import 済み）

`index.html` はプロジェクトルートにあり、Vite 標準のエントリ方式（`<script type="module"
src="/src/main.ts">`）。**Vue は npm 依存からバンドル済み**で、`vite.config.ts` で
`vue/dist/vue.esm-bundler.js`（コンパイラ同梱）へエイリアスしている（inline テンプレートを
実行時コンパイルするため）。`lodash`/`emojione` のみ CDN グローバル external のまま。
`npm run dev` は **真の HMR** が効く（SFC を編集すると即反映）。

全面 SFC 化が進んで inline テンプレート（DOM テンプレート + x-template モーダル）が無くなれば、
実行時コンパイラが不要になるので、`vite.config.ts` の vue エイリアスを外して **runtime-only
ビルドへ戻し、バンドルを小さくできる**（現状 +コンパイラで ~310KB → ~200KB 程度に）。これは
SFC 化完了後の最後の最適化。
...
3. `index.html` の元マークアップを `<xxx ... />` 使用に置換
...
> 必ず `close()`** する（常駐させると次のビルドとポート衝突する）。`npm run dev`（Vite dev サーバ）は
...
- ビルド/dev: `vite.config.ts`（Vite 標準エントリ。dev は HMR 付き `vite`）
