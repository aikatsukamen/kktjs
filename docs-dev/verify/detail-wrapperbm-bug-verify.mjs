// detail-wrapperbm-bug-verify.mjs — updateWrapperBM([status], 'detail') が
// status 本体に media_opened/content_opened を設定できているかを検証。
// 配列に setプロパティしてるのでは？という疑惑の確認。
...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    // 実コードと同じ呼び出し方
    const arrayArg = [status];
    if (app.updateWrapperBM) app.updateWrapperBM(arrayArg, 'detail');
    
    return {
      // 期待: status 本体に media_opened/content_opened がセットされている
      statusMediaOpened: status.media_opened,
      statusContentOpened: status.content_opened,
      // 実際: 配列にプロパティがセットされた?
      arrayMediaOpened: arrayArg['media_opened'],
      arrayContentOpened: arrayArg['content_opened'],
      arrayIsArray: Array.isArray(arrayArg),
      arrayLen: arrayArg.length,
      arrayItem0Same: arrayArg[0] === status,
    };
  });

  console.log('=== updateWrapperBM([status], "detail") bug investigation ===\n');
  console.log('  status.media_opened   (should be true):', result.statusMediaOpened);
  console.log('  status.content_opened (should be true):', result.statusContentOpened);
  console.log('');
  console.log('  arrayArg.media_opened  (sign of bug):', result.arrayMediaOpened);
  console.log('  arrayArg.content_opened(sign of bug):', result.arrayContentOpened);
  console.log('');
  console.log('  array still has status at [0]:', result.arrayItem0Same);
  console.log('  array length:', result.arrayLen);

  if (result.arrayMediaOpened === true && result.statusMediaOpened !== true) {
    console.log('\n  ✗✗✗ BUG CONFIRMED: setting media_opened on the array, not the status itself!');
    console.log('  → "Open All Wrappers" setting does NOT affect detail page status display');
  } else if (result.statusMediaOpened === true) {
    console.log('\n  ✓ status got media_opened set correctly');
  } else {
    console.log('\n  ? unexpected state');
  }
