...
│   ├── js/                 ベンダ JS（vue 3 グローバル / lodash / emojione / picker 等）
│   ├── sw.js  manifest.json  favicon.ico
├── docs/                 ★ビルド成果物（GitHub Pages 公開元・git 管理しない）
├── vite.config.ts        Vite 設定（本番ビルド。SFC・external グローバル解決）
├── dev.mjs               dev サーバ（Vite watch ビルド + 軽量プレビュー）
├── build.config.mjs      共有ビルド設定（Vite / esbuild 双方が参照）
├── build.mjs             esbuild ビルド（フォールバック。SFC 非対応）
...
編集対象は `src/`（TS / SFC）と `public/`（静的ファイル）。
...
`vue-tsc`。Vue 3 / lodash / emojione は `public/index.html` で個別 `<script>` 読み込み
される CDN グローバルであり、バンドルには含めない（external 扱い。`vite.config.ts` の
仮想モジュールで `import` を `window.Vue` 等へ解決する）。`vue` は npm 依存にも入れて
あるが、これは **SFC の型付けとコンパイル時参照のため**で、実行時は CDN グローバルを使う。
...
```bash
npm run build        # 本番ビルド（Vite → docs/。SFC もコンパイル）
npm run dev          # Vite watch ビルド + 軽量プレビュー（http://localhost:5180/）
npm run preview      # docs/ を Vite preview で配信
npm run typecheck    # vue-tsc 型チェック（.ts + .vue、出力なし）
npm run build:esbuild # フォールバック（esbuild。SFC 非対応。.vue を使うと失敗）
```

ビルド設定の中核（エントリ/出力/external）は `build.config.mjs` に集約し、Vite
（`vite.config.ts`）と esbuild（`build.mjs`）の双方が参照する。Vite は `src/main.ts` を
IIFE バンドルして `docs/js/main.js` に出力し（既存 index.html の `<script src="js/main.js">`
構成を維持）、`public/` を `docs/` へコピー、SFC の `<style scoped>` を `docs/style.css`
へ抽出する（index.html がこれを読む）。
...
```bash
npm run dev   # → http://localhost:5180/（src/・public/ の変更で docs/ を自動リビルド）
```

完全な HMR が必要になったら、`index.html` を Vite エントリ方式へ作り替える（CDN グローバルを
やめ、`vue` を `import` に統一、`<script type="module" src="/src/main.ts">` にする）。これは
別タスク。詳細は [`docs-dev/VUE3_MIGRATION.md`](docs-dev/VUE3_MIGRATION.md)。

> **`public/index.html` の inline テンプレート（約3,790行）を `.vue` へ全面移行**する場合の
> 引き継ぎは [`docs-dev/SFC_MIGRATION.md`](docs-dev/SFC_MIGRATION.md) にまとめてある
> （切り出し単位・最優先の `Status` コンポーネント・このコード固有の罠・1コンポーネント
> 移行手順・検証方法）。SFC 自体は `src/components/VersionBadge.vue` が動作実証済み。
...
**Vue 3 へ移行済み**（`public/js/vue.min.js` は Vue 3 のグローバルビルド
`vue.global.prod.js`、`window.Vue` グローバル）。Vue 2 は 2023年末で EOL のため
移行した。対応した破壊的変更は次のとおり（詳細・背景は
...
- **名前付きスロット**: `slot="header"` → `<template #header>`（`public/index.html` の
...
`v1.7.1_node24_1`）をリリースごとに変更**すること。これは元実装の運用を踏襲している。
